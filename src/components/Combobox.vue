...
const isRemovingOption = ref(false)

defineExpose({selectedOptions})

const inputDisabled = computed(() => {
    return selectedOptions.value.length >= props.max && !isRemovingOption.value
})

const optionDisabled = (option) => {
    return selectedOptions.value.length >= props.max && !selectedOptions.value.includes(option)
}
